...
where
  ...
  {candidateidsql}
    ...
    def __format_plume(self, row, s3url):
        plume = {
            "id": row[PLUME_ID],
            "json_url": row[JSON_URL].replace("{s3}", s3url),
            "png_url": row[PNG_URL].replace("{s3}", s3url),
            "plume_url": row[PLUME_URL].replace("{s3}", s3url),
            "rgbqlctr_url": row[RGBQLCTR_URL].replace("{s3}", s3url),
            "png_url_thumb": row[PNG_URL_THUMB].replace("{s3}", s3url),
            "plume_url_thumb": row[PLUME_URL_THUMB].replace("{s3}", s3url),
            "rgbqlctr_url_thumb": row[RGBQLCTR_URL_THUMB].replace("{s3}", s3url),
            "plume_tiff_url": row[PLUME_TIFF_URL].replace("{s3}", s3url),
            "rgb_tiff_url": row[RGB_TIFF_URL].replace("{s3}", s3url),
...
limit %s;
    ...
    def __format_flight_basic(self, row, s3url):
        flight = {
            "name": row[FLIGHT_NAME].replace("{s3}", s3url),
            "shape": [],
    ...
    def __format_flight_geojson(self, row, s3url):
        ...
        geojson["properties"] = {
            ...
            "png_url": row[FLIGHT_IMAGE_URL].replace("{s3}", s3url),
            "data_date_dt": row[FLIGHT_TIMESTAMP],
            "id": row[FLIGHTLINE_ID]
        }
        return geojson
