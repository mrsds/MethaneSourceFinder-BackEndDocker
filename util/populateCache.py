# Run this script to populate the detection stats database table to speed up queries for detection stats
# This script takes a while to run (~24 hrs) but will determine whether a date range has already been calculated and only do those that are needed
# Also, this could theoretically be run with multiple parallel instances if the database can handle it.
